...
__all__ = ['UniqueList', 'Labeller', 'Labeller', 'lfs_order', 'tasks_labels']
...
def listen_lfs_order(self:Labeller):
    self._lfs_order = L()
    for sub in self.subs: sub.listen_one(self._lfs_order.append)

# Cell
@patch_property
def lfs_order(self:Labeller):
    for sub in self.subs: sub.listen_one(None)
